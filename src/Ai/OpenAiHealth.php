 ...
 * OpenAiProvider elé — SZÁNDÉKOSAN külön osztály, ugyanazzal a mintával
 * és indoklással, mint OllamaHealth/AnthropicHealth (lásd ott): az
 * UI-nak nem szabad minden oldalbetöltéskor valódi hálózati/API-hívást
 * indítania.
 */
final class OpenAiHealth
{
    ...
    public static function check(
        ...
        $provider = new OpenAiProvider($baseUrl, $apiKey, $model, $timeoutSeconds);
    ...
    private static function cacheFile(string $baseUrl, string $apiKey, string $model): string
    {
        ...
        return $dir . '/openai-' . hash('sha256', $baseUrl . '|' . $apiKey . '|' . $model) . '.json';
    }
