...
/**
 * A kör 20. pontjának VALÓDI HTTP-n bizonyított követelménye az
 * AnomalyAgent-re (Fázis 5) — ugyanaz a minta, mint
 * tests/AiSalesClientProxyHttpTest.php (SalesAgent)/
 * tests/AiClientProxyHttpTest.php (InventoryAgent): a Kliens SOSE hív
 * Ollamát/semmilyen providert közvetlenül, SOSE futtatja az AnomalyAgent-et
 * helyben, az /api/ai-anomaly.php végpont a MEGLÉVŐ ClientProxy-n
 * keresztül megy, és a tényleges agent-futás a Szerveren történik —
 * bizonyítva, hogy a _bootstrap.php node_role-elágazása agent-
 * függetlenül, MINDHÁROM agentre véd, nincs új Kliens-oldali védő kód az
 * AnomalyAgent-hez.
 */
final class AiAnomalyClientProxyHttpTest extends TestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        self::$ollamaRoot = sys_get_temp_dir() . '/sm_ai_anomaly_proxy_ollama_' . bin2hex(random_bytes(6));
...
if ($path === '/api/tags') {
    ...
    echo json_encode(['message' => ['role' => 'assistant', 'content' => 'Szerver-oldali anomália-válasz Kliens-kérésre.']]);
} else {
...
PHP);
        ...
        self::$serverRoot = sys_get_temp_dir() . '/sm_ai_anomaly_proxy_server_' . bin2hex(random_bytes(6));
        ...
        $db->saveStaff(['name' => 'Anomaly Proxy Teszt Admin', 'pin' => '55226', 'role' => 'admin']);
        $client = $db->registerClient('Anomaly AI proxy teszt kliens');
        unset($db);
        ...
        self::directRequest(self::$serverPort, 'POST', '/api/staff-login.php', ['pin' => '55226'], ['X-CSRF-Token' => $csrf], $adminJar);
        ...
        self::$clientRoot = sys_get_temp_dir() . '/sm_ai_anomaly_proxy_client_' . bin2hex(random_bytes(6));
    ...
    public function testClientAnomalyRequestIsProxiedAndAnsweredByServer(): void
    {
        ...
        self::directRequest(self::$clientPort, 'POST', '/api/staff-login.php', ['pin' => '55226'], ['X-CSRF-Token' => $csrf], $clientJar);
        $csrf2 = self::directRequest(self::$clientPort, 'GET', '/api/auth-status.php', null, [], $clientJar)['json']['csrf_token'];

        $res = self::directRequest(self::$clientPort, 'POST', '/api/ai-anomaly.php', ['message' => 'Van valami szokatlan?'], ['X-CSRF-Token' => $csrf2], $clientJar);

        $this->assertSame(200, $res['status'], $res['body']);
        $this->assertTrue($res['json']['ok'] ?? false);
        $this->assertSame('anomaly', $res['json']['agent']);
        $this->assertSame('Szerver-oldali anomália-válasz Kliens-kérésre.', $res['json']['answer']);
    }
    ...
    public function testOllamaWasOnlyEverHitByTheServerNeverConfiguredOnTheClient(): void
    {
        ...
        $this->assertNotEmpty($hits, 'A stub Ollamának kapnia kellett legalább egy hívást — a Szerver oldali AnomalyAgent-futástól.');
