...
/**
 * A kör 11. pontjának VALÓDI HTTP-n bizonyított követelménye: a Kliens
 * SOSE hív Ollamát közvetlenül, az AI-végpont a MEGLÉVŐ ClientProxy-n
 * keresztül megy, és a tényleges agent-futás a Szerveren történik.
 * Valódi Kliens+Szerver app-másolat, két külön `php -S` folyamat —
 * ugyanaz a minta, mint tests/ClientProxyHttpTest.php.
 *
 * A Kliens node-nak SZÁNDÉKOSAN nincs semmilyen AI/Ollama-konfigurációja
 * (a saját settings.json-ja sose is jön létre) — ez önmagában bizonyítja,
 * hogy a funkció kizárólag szerver-oldalon fut: ha a Kliens bármikor
 * megpróbálna saját maga Ollamát hívni, nem is tudná, milyen URL-t
 * használjon.
 */
final class AiClientProxyHttpTest extends TestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        // --- Stub "Ollama" — csak azt méri, hányszor hívták, és egy
        // egyszerű, eszköz-hívás nélküli végleges választ ad. ---
        self::$ollamaRoot = sys_get_temp_dir() . '/sm_ai_proxy_ollama_' . bin2hex(random_bytes(6));
...
if ($path === '/api/tags') {
    ...
    echo json_encode(['message' => ['role' => 'assistant', 'content' => 'Szerver-oldali válasz Kliens-kérésre.']]);
} else {
...
PHP);
        ...
        self::$serverRoot = sys_get_temp_dir() . '/sm_ai_proxy_server_' . bin2hex(random_bytes(6));
        ...
        $db->saveStaff(['name' => 'Proxy Teszt Admin', 'pin' => '55221', 'role' => 'admin']);
        $client = $db->registerClient('AI proxy teszt kliens');
        unset($db);

        // Admin munkamenet közvetlenül a Szerveren, hogy beállíthassuk az AI-t.
        self::$adminJar = self::$serverRoot . '/admin-cookies.txt';
        self::directRequest(self::$serverPort, 'GET', '/api/auth-status.php', null, [], self::$adminJar);
        ...
        self::directRequest(self::$serverPort, 'POST', '/api/login.php', ['password' => 'szerver-teszt-jelszo'], [], self::$adminJar);
        $csrf = self::directRequest(self::$serverPort, 'GET', '/api/auth-status.php', null, [], self::$adminJar)['json']['csrf_token'];
        self::directRequest(self::$serverPort, 'POST', '/api/staff-login.php', ['pin' => '55221'], ['X-CSRF-Token' => $csrf], self::$adminJar);
        $csrf2 = self::directRequest(self::$serverPort, 'GET', '/api/auth-status.php', null, [], self::$adminJar)['json']['csrf_token'];
        ...
        self::$clientRoot = sys_get_temp_dir() . '/sm_ai_proxy_client_' . bin2hex(random_bytes(6));
    ...
    public function testClientRequestIsProxiedAndAnsweredByServer(): void
    {
        ...
        $this->assertSame('Szerver-oldali válasz Kliens-kérésre.', $res['json']['answer']);
    }
    ...
    public function testOllamaWasOnlyEverHitByTheServerNeverConfiguredOnTheClient(): void
    {
        ...
        // helyi AI-beállítás, amit egyáltalán fel tudna használni.
        $this->assertFileDoesNotExist(self::$clientRoot . '/data/settings.json');
        $this->assertFileDoesNotExist(self::$clientRoot . '/data');

        // A stub Ollama ténylegesen kapott hívásokat — de ezek KIZÁRÓLAG a
        // Szerver saját folyamatától jöhettek (a Kliens sose ismeri az
        // Ollama URL-jét, azt csak a Szerver settings.json-ja tartalmazza).
        $hits = @file_get_contents(self::$ollamaRoot . '/hits.log');
        $this->assertNotEmpty($hits, 'A stub Ollamának kapnia kellett legalább egy hívást — a Szerver oldali agent-futástól.');
