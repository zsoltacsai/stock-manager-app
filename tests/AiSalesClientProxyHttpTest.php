...
/**
 * A kör 11. pontjának VALÓDI HTTP-n bizonyított követelménye a
 * SalesAgent-re (Fázis 4) — ugyanaz a minta, mint
 * tests/AiClientProxyHttpTest.php (InventoryAgent): a Kliens SOSE hív
 * Ollamát/semmilyen providert közvetlenül, SOSE futtatja a SalesAgent-et
 * helyben, az /api/ai-sales.php végpont a MEGLÉVŐ ClientProxy-n keresztül
 * megy, és a tényleges agent-futás a Szerveren történik — bizonyítva,
 * hogy a _bootstrap.php node_role-elágazása agent-függetlenül,
 * MINDKÉT agentre (Inventory ÉS Sales) véd, nincs új Kliens-oldali védő
 * kód a SalesAgent-hez.
 */
final class AiSalesClientProxyHttpTest extends TestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        self::$ollamaRoot = sys_get_temp_dir() . '/sm_ai_sales_proxy_ollama_' . bin2hex(random_bytes(6));
...
if ($path === '/api/tags') {
    ...
    echo json_encode(['message' => ['role' => 'assistant', 'content' => 'Szerver-oldali forgalmi válasz Kliens-kérésre.']]);
} else {
...
PHP);
        ...
        self::$serverRoot = sys_get_temp_dir() . '/sm_ai_sales_proxy_server_' . bin2hex(random_bytes(6));
        ...
        $db->saveStaff(['name' => 'Sales Proxy Teszt Admin', 'pin' => '55225', 'role' => 'admin']);
        $client = $db->registerClient('Sales AI proxy teszt kliens');
        unset($db);
        ...
        self::$clientRoot = sys_get_temp_dir() . '/sm_ai_sales_proxy_client_' . bin2hex(random_bytes(6));
    ...
    public function testClientSalesRequestIsProxiedAndAnsweredByServer(): void
    {
        ...
        self::directRequest(self::$clientPort, 'POST', '/api/staff-login.php', ['pin' => '55225'], ['X-CSRF-Token' => $csrf], $clientJar);
        $csrf2 = self::directRequest(self::$clientPort, 'GET', '/api/auth-status.php', null, [], $clientJar)['json']['csrf_token'];

        $res = self::directRequest(self::$clientPort, 'POST', '/api/ai-sales.php', ['message' => 'Mennyi volt a forgalom?'], ['X-CSRF-Token' => $csrf2], $clientJar);

        $this->assertSame(200, $res['status'], $res['body']);
        $this->assertTrue($res['json']['ok'] ?? false);
        $this->assertSame('sales', $res['json']['agent']);
        $this->assertSame('Szerver-oldali forgalmi válasz Kliens-kérésre.', $res['json']['answer']);
    }
    ...
    public function testOllamaWasOnlyEverHitByTheServerNeverConfiguredOnTheClient(): void
    {
        ...
        $this->assertNotEmpty($hits, 'A stub Ollamának kapnia kellett legalább egy hívást — a Szerver oldali SalesAgent-futástól.');
    ...
    {
        // A Kliens teljes fájlrendszeri másolatában (data/ sose jött
        // létre) nincs semmi, ami a Szerver oldali kliens-titkot
        // tartalmazná — a Kliens csak a SAJÁT (config/installer-
        // generated.php-ban tárolt) client_secret-jét ismeri, ami a
        // ClientProxy-hitelesítéshez kell, nem a Szerver AI-beállításait.
        $this->assertFileDoesNotExist(self::$clientRoot . '/data');
    }
}
