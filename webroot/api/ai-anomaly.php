...
// A harmadik AI-agent UGYANAZZAL az admin-jogszinthez kötéssel, mint az
// Inventory-/SalesAgent (lásd webroot/api/ai-inventory.php azonos
// indoklása) — egy AI-végpont sem enged üzletileg érzékeny adatot
// egyszerű pénztárosi jogszinttel.
require_admin($db);
...
$agent = new AnomalyAgent($provider, $db, $appSettings, max(1, (int) $appSettings['ai_max_iterations']));
...
if (!$result->success) {
    ...
    send_json(['ok' => false, 'agent' => AnomalyAgent::name(), 'error' => $result->error, 'tools_used' => $result->toolsUsed], 502);
}

send_json([
    'ok' => true,
    'agent' => AnomalyAgent::name(),
